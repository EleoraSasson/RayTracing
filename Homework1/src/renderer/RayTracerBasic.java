...
public class RayTracerBasic extends RayTracerBase {
	 ...
	 */
	/*
	 * private Double3 transparency(GeoPoint geoPoint, LightSource ls, Vector l,
	 * Vector n) { Vector directionToLight = l.scale(-1); // to change direction of
	 * vector to be from point to light Ray rayPointToLight = new
	 * Ray(geoPoint.point, directionToLight, n); double lightDistance =
	 * ls.getDistance(geoPoint.point); List<GeoPoint> intersections =
	 * scene.geometries.findGeoIntersections(rayPointToLight, lightDistance);
	 * Double3 ktr = Double3.ONE; // if there is no object between point and light,
	 * it is fully transparent if (intersections == null) return Double3.ONE; for
	 * (GeoPoint gp : intersections) { // diminish the transparency based on the
	 * transparency of the geometry ktr = ktr.product(gp.geometry.getMaterial().kT);
	 * 
	 * // if it's mostly opaque, make it fully opaque if
	 * (ktr.lowerThan(MIN_CALC_COLOR_K)) { return Double3.ZERO; } } return ktr; }
	 */
	 ...
	 */
	/*
	 * private Ray constructRefractedRay(Vector n, Point p, Ray inRay) { // move
	 * original point along normal so be new starting point for reftactedRay return
	 * new Ray(p, inRay.dir, n); }
	 */
	 ...
	 * calculate direction of reflectedRay double d1 = inRay.dir.dotProduct(n); if
	 * (isZero(d1)) { return null; } Vector v1 = n.scale(d1 * 2); Vector newV =
	 * inRay.dir.subtract(v1); // put together to get reflectedRay return new Ray(p,
	 * newV, n); }
	 */
